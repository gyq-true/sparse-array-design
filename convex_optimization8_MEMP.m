...
% =============================================================
%研二下改11版，将第二步坐标优化变成增广矩阵束方法
% =============================================================
%因为MEMP倾向于将阵元坐标推向边界以保持阵列孔径。或者恢复锁定点？
%下一步尝试在第二部削减阵元数目，将Q_size调小？
%凸优化对不合适坐标削减激励即可，为什么优化失败了
%削除重复阵元？以及在MEMP中增加权重防止挤在阵列边缘？
% =============================================================
%措施：% 更新坐标处加入重心回归（有效/无效）
% =============================================================
clear; clc; close all;
...
%RSVD
p = 5;          % 过采样参数 (Over-sampling parameter)，通常取 5-10，提高捕捉信号子空间的概率
q = 2;          % 功率迭代次数 (Power iterations)，用于奇异值衰减较慢的情况，增加精度
...
cvx_end
    % 如果 CVX 失败，跳出循环
%     is_locked = false(size(w)); 
%     is_locked(lock_idx) = true;
    if strcmp(cvx_status, 'Infeasible') || strcmp(cvx_status, 'Failed')
        fprintf('\n[警告] 迭代%d阵元激励求解失败。',iter);
        break;
    else
        keep_idx = abs(w_new) >  eta * max(abs(w_new));
%         keep_idx(lock_idx) = true; % 无论激励多小，锁定点必须保留
        if sum(keep_idx)<7
            fprintf('\n[警告]迭代%d阵元激励求解后阵元太少，停止迭代。',iter);
            break;
        else
        w = w_new(keep_idx);%更新变量,缩减向量长度
        pos_x = pos_x(keep_idx);
        pos_y = pos_y(keep_idx);
%         is_locked_new = is_locked(keep_idx); 
%         lock_idx = find(is_locked_new); % 重新获取新的绝对索引
        fprintf('Iteration %d:阵元激励优化Status = %s,阵元数目=%d\n', iter,  cvx_status,sum(keep_idx));
        end
    end
            
    %% --- 步骤 B: 利用 MEMP 方式重构阵元坐标  ---
    % 1. 将步骤 A 得到的方向图 重新排列为 2D 矩阵 (u-v 网格)
    AF_B= (exp(1j * k * (pos_x * u_flat.' + pos_y * v_flat.'))).' * w;%2500*1
    AF_2D = reshape(AF_B, spacefrequency, spacefrequency);

    % 2. 构造 2D 增强 Hankel 矩阵 (论文 Eq. 4-5)
    % 选取窗口大小 L_e, M_e (通常取采样点数的一半左右)
    M_grid= spacefrequency; 
    N_grid= spacefrequency; 

    du=u_vec(2)-u_vec(1);
    dv=v_vec(2)-v_vec(1);
    
    K_e= floor(M_grid/2);  %原文中的K
    L_e = floor(N_grid/2);  %原文中的L
    
    % 构造 Hankel 块矩阵
    sub_hankels = cell(1, M_grid);
    for m = 1:M_grid
        column_data = AF_2D(:, m); % 取第 m 列数据
        % 构造 L_e x (N_grid - L_e + 1) 的 Hankel 矩阵
        sub_hankels{m} = hankel(column_data(1:L_e), column_data(L_e:end));%X_m子块
    end
    H = [];
    for i = 1 : (M_grid - K_e + 1)
        row_of_blocks = [];
        for j = i : i + K_e - 1
            % 按照论文排列，将子块竖直拼接
            row_of_blocks = [row_of_blocks; sub_hankels{j}]; 
        end
        % 将每一层大的块组水平拼接
        H = [H, row_of_blocks]; %625*676
    end

    %--- 3. 提取主特征向量 (使用 RSVD 加速) ---
    Q_size= sum(keep_idx); % 设定目标秩 k (即你想提取的阵元数 N_total)
    [m_H, n_H] = size(H);
    Omega = randn(n_H, Q_size + p);% 生成随机高斯矩阵 Omega
    Y = H * Omega;   % 形成采样矩阵 Y，并进行功率迭代 (提高数值稳定性)
    for j = 1:q
        % 通过 (H * H')^q 迭代来拉大奇异值之间的间距，使信号特征更明显
        [Y, ~] = qr(Y, 0); % 迭代中进行 QR 保持数值稳定性
        Y = H * (H' * Y);
    end
    [Q, ~] = qr(Y, 0);% 对采样矩阵进行正交化，得到投影基 Q
    B = Q' * H;% 将高维矩阵 H 投影到低维空间 B (B 的规模远小于 H)
    [U_tilde, S_h, ~] = svd(B, 'econ');% 对小矩阵 B 进行标准 SVD 分解
    U_h = Q * U_tilde;%  恢复原始矩阵 H 的左奇异向量 (信号子空间)
    Us = U_h(:, 1:Q_size); % 提取前 Q_size 个主特征向量


    % 4. 矩阵笔法提取极点 (Matrix Pencil)
    % X 方向算子 (利用位移不变性) U_s=E_L*T_1
    U_x1 = Us(1:end-K_e, :); 
    U_x2 = Us(K_e+1:end, :);
    phi_x = pinv(U_x1) * U_x2;%F_1,与Y_d矩阵相似，具有相同的特征值
    [Ex, Dx] = eig(phi_x);
    lambda_x = diag(Dx); % X方向极点

    % Y 方向算子
    [total_rows, ~] = size(Us);%total_rows为625
    Us_P = zeros(size(Us));
    for q = 1:Q_size
        temp_block = reshape(Us(:, q), [L_e, K_e]);
        temp_transposed = temp_block'; 
    Us_P(:, q) = temp_transposed(:); %子空间重置
    end
    rows_new = size(Us_P, 1);%相当于row_total,625
    E1_y = [eye(rows_new - 1), zeros(rows_new - 1, 1)];
    E2_y = [zeros(rows_new - 1, 1), eye(rows_new - 1)];
    PU_lower = E1_y * Us_P; % 相当于 Us_P(1:end-Le, :)
    PU_upper = E2_y * Us_P; % 相当于 Us_P(Le+1:end, :)
    F2 = pinv(PU_lower) * PU_upper;
    [Ez, Dz] = eig(F2);
    lambda_y = diag(Dz);

    % 6. 将极点映射回物理坐标 (x, y)
    new_pos_x = angle(lambda_x) / (k * du); 
    new_pos_y = angle(lambda_y) / (k * dv);
    %坐标和激励重排,应保持原阵列的顺序
    old_pos_x=pos_x;old_pos_y=pos_y;
    [dx_recovered, dy_recovered] = extended_matching_algorithm2(old_pos_x, old_pos_y, Q_size, new_pos_x, new_pos_y, AF_B, u_flat, v_flat, w, k);
%     % 7. 限制在原始区域内(MEMP 是解析解，需手动加入限制)
%     dx_recovered = max(min(dx_recovered, x_max_limit), x_min_limit);
%     dy_recovered = max(min(dy_recovered, y_max_limit), y_min_limit);
    % 更新坐标（除锁定点以外）,加入重心回归
    beta=0.8;
    pos_x = old_pos_x+beta*(dx_recovered-old_pos_x);
    pos_y = old_pos_y+beta*(dy_recovered-old_pos_y);
    
%     pos_x(lock_idx) = old_pos_x(lock_idx);
%     pos_y(lock_idx) = old_pos_y(lock_idx);
    
    fprintf('Iteration %d: MEMP 坐标重构完成\n', iter);
